...
WHERE p.user_id IN (SELECT user_id FROM profiles WHERE 
	(birthday BETWEEN '2011-02-19' AND '2023-02-19') OR
	(birthday = '2011-02-19') OR (birthday = '2023-02-19'));


-- Определить кто больше поставил лайков (всего): мужчины или женщины.
/* SELECT
	COUNT(l.user_id) AS 'Количество лайков'
	FROM likes l
	...
	FROM likes l
	JOIN profiles p ON p.user_id = l.user_id
	WHERE p.gender = 'm'), SELECT'Женщины', SELECT 'Мужчины'); */
